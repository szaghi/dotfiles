...
if exists("fortran_fold")
  ...
  if (b:fortran_fixed_source == 1)
    ...
    syn region fortranInterface transparent fold keepend extend start="^\s*associate\>" skip="^\s*[!#].*$" excludenl end="\<end\s*associate\>" contains=ALLBUT,fortranProgram,fortranModule
  endif
