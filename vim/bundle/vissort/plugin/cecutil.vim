...
" ---------------------------------------------------------------------
" DispVarSet: this function supports the notion of a "display variable" {{{2
"             If the windows are not moved about, then this is somewhat
"             equivalent to a w:var ; however, w:var's don't track the
"             buffers being moved to different windows.
fun! DispVarSet(varname,value)
"  call Dfunc("DispVarSet()")
"  call Dret("DispVarSet")
endfun

" ---------------------------------------------------------------------
" DispVarGet: get the value of a DispVar {{{2
fun! DispVarGet(varname)
"  call Dfunc("DispVarGet()")
"  call Dret("DispVarGet")
endfun

" ---------------------------------------------------------------------
" DispVarFree: free/unset/unlet/destroy a DispVar {{{2
fun! DispVarFree(varname)
"  call Dfunc("DispVarFree()")
"  call Dret("DispVarFree")
endfun
