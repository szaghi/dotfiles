...
### CLI conventions (FoBiS.py 3.8+)

The CLI binary is `fobis` (the legacy `FoBiS.py` name remains as an alias, but `fobis` is canonical). Flags use **double-dash long form** (`--mode`, `--lmodes`, `--ex`, `--ls`). The old single-dash short forms (`-mode`, `-lmodes`, `-ex`) are no longer accepted in 3.8+. When writing or updating fobos-related scripts, CI, or docs, always use:

- `fobis build --mode <name>`     (not `FoBiS.py build -mode <name>`)
- `fobis build --lmodes`          (not `-lmodes`)
- `fobis rule --ex <rule>`        (not `-ex`)
- `fobis rule --ls`               (not `-ls`)
- `fobis fetch`                   (no change)
